...
class DrawLots(lotsPath : String) {
    ...
    init {
        ...
        try {
            lots = DrawLotsNew.getResourcesConfig(lotsPath).getConfigSectionList("Lots")
            DrawLotsNew.logger.info("本地抽签数据加载成功,数据大小为：" + lots!!.size)
        } catch (e: Exception) {
            e.printStackTrace()
            DrawLotsNew.logger.info("无法加载本地抽签数据")
            lots = null
        }
